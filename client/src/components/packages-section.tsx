...
export default function PackagesSection() {
  return (
    <motion.section
      id="packages"
      className="relative bg-charcoal py-20 z-40 shadow-2xl"
      initial={{ y: 100 }}
      whileInView={{ y: 0 }}
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="font-playfair text-4xl md:text-5xl font-bold text-white mb-4" data-testid="text-packages-title">
            Wedding Packages
          </h2>
          <p className="text-lg text-gray-300 max-w-3xl mx-auto" data-testid="text-packages-subtitle">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {packages.map((pkg, index) => (
            <motion.div
              key={pkg.id}
              className={`rounded-3xl p-8 shadow-xl hover:shadow-2xl transition-all duration-300 ${
                pkg.featured
            ...
            >
              {pkg.featured && (
                <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-champagne text-charcoal px-4 py-2 rounded-full text-sm font-medium">
                  Most Popular
                </div>
              )}
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h3 className="font-playfair text-2xl font-bold text-white text-center mb-8" data-testid="text-addons-title">
            Popular Add-Ons
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {addOns.map((addOn, index) => (
              <div key={index} className="text-center text-white" data-testid={`addon-${index}`}>
                <addOn.icon className="w-8 h-8 text-champagne mb-3 mx-auto" />
                <h4 className="font-semibold mb-2" data-testid={`text-addon-title-${index}`}>
                  {addOn.title}
                </h4>
                <p className="text-cream/70 text-sm mb-2" data-testid={`text-addon-description-${index}`}>
                  {addOn.description}
                </p>
